...
export default function DailyLeaderboardsPage({ loaderData }: Route.ComponentProps) {
 ...
 const prevUrl =
  LINK.PRODUCT_LEADERBOARDS_REDIRECT('monthly') + `/${prevMonth.weekYear}/${prevMonth.month}`;
 const nextUrl =
  LINK.PRODUCT_LEADERBOARDS_REDIRECT('monthly') + `/${nextMonth.weekYear}/${nextMonth.month}`;
