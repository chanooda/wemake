...
import { weeklySchema } from '~/common/model/req.schema';
import {
...
export const meta: Route.MetaFunction = ({ params }) => {
 ...
 if (success) {
  ...
  title = `The best products of ${date.startOf('week').toLocaleString(DateTime.DATE_SHORT)} - ${date
   .endOf('week')
...
export default function DailyLeaderboardsPage({ loaderData }: Route.ComponentProps) {
 ...
 const prevUrl =
  LINK.PRODUCT_LEADERBOARDS_REDIRECT('weekly') + `/${prevDate.weekYear}/${prevDate.weekNumber}`;
 const nextUrl =
  LINK.PRODUCT_LEADERBOARDS_REDIRECT('weekly') + `/${nextDate.weekYear}/${nextDate.weekNumber}`;
   ...
   <div className="mx-auto flex w-full max-w-screen-md flex-col gap-4">
    <div className="flex w-full items-center justify-center gap-4">
     <Button variant="ghost" asChild>
      <Link to={prevUrl}>&larr; {prevDate.toLocaleString(DateTime.DATE_SHORT)}</Link>
     </Button>
     {!isToday && (
      <Button variant="ghost" asChild>
       <Link to={nextUrl}>{nextDate.toLocaleString(DateTime.DATE_SHORT)} &rarr;</Link>
      </Button>
     )}
    </div>
