...
const CommunityPage = ({ loaderData: { posts, topics } }: Route.ComponentProps) => {
 const [searchParams, setSearchParams] = useSearchParams();

 const sorting = searchParams.get('sort') || SORT_OPTIONS[0].label;
 const period =
  PERIOD_OPTIONS.find((option) => option.value === searchParams.get('period'))?.label ||
  PERIOD_OPTIONS[0].label;
  ...
  <div>
   ...
   <div className="grid grid-cols-6 gap-48">
       ...
       <div className="flex items-center gap-4">
        <DropdownMenu>
         <DropdownMenuTrigger className="flex items-center gap-2">
          <span className="capitalize">{sorting}</span> <ChevronDownIcon className="size-5" />
         </DropdownMenuTrigger>
         <DropdownMenuContent>
        ...
        <DropdownMenu>
         <DropdownMenuTrigger className="flex items-center gap-2">
          <span className="capitalize">{period}</span> <ChevronDownIcon className="size-5" />
         </DropdownMenuTrigger>
         <DropdownMenuContent>
    ...
    <aside className="col-span-2">
     <h3 className="text-muted-foreground text-lg font-semibold">TOPICS</h3>
